...
const AIMARAJS_BASEFOLDER = "AimaraJS/"
	...
	var tree = {
		...
		createNode : function(p_text, p_expanded, p_icon, p_parentNode, p_tag,
			...
			var node = {
				...
				expanded : p_expanded,
				childNodes : [],
				path : p_parentNode == null ? p_text
						: (p_parentNode.path + "/" + p_text),
				tag : p_tag,
				contextMenu : p_contextmenu,
				elementLi : null,
		...
		drawNode : function(p_ulElement, p_node) {
			...
			if (p_node.childNodes.length == 0) {
				v_exp_col = createImgElement('toggle_off', 'exp_col',
						AIMARAJS_BASEFOLDER + 'images/collapse.png');
				...
				if (p_node.expanded) {
					...
					v_exp_col = createImgElement('toggle_on', 'exp_col',
							AIMARAJS_BASEFOLDER + 'images/expand.png');
				}
			}
		...
		expandNode : function(p_node) {
			if (p_node.childNodes.length > 0 && p_node.expanded == false) {
				...
				img.id = "toggle_off";
				img.src = AIMARAJS_BASEFOLDER + 'images/collapse.png';
		...
		collapseNode : function(p_node) {
			if (p_node.childNodes.length > 0 && p_node.expanded == true) {
				...
				img.id = "toggle_on";
				img.src = AIMARAJS_BASEFOLDER + 'images/expand.png';
				elem_ul = img.parentElement.getElementsByTagName("ul")[0];
		...
		// 
		// given a node path it selects it in the tree
		...
		nodeContextMenu : function(p_event, p_node) {
			if (p_event.button == 2) {
				...
				if (p_node.contextMenu != undefined) {
					...
					for (var i = 0; i < v_menu.elements.length; i++)
						(function(i) {
							...
							var v_ul = createSimpleElement('ul', null,
									'sub-menu');

							v_a.appendChild(document
									.createTextNode(v_menu.elements[i].text));

							v_li.appendChild(v_span);

							if (v_menu.elements[i].icon != undefined) {
								var v_img = createImgElement('null', 'null',
										v_menu.elements[i].icon);
							...
							if (v_menu.elements[i].submenu != undefined) {
								var v_span_more = createSimpleElement('div',
										null, null);
								v_span_more.appendChild(createImgElement(null,
										'menu_img', AIMARAJS_BASEFOLDER
												+ 'images/right.png'));
								v_li.appendChild(v_span_more);
								v_tree.contextMenuLi(
										v_menu.elements[i].submenu, v_ul,
										p_node);
							}
